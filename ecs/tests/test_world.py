...
def test_add_processor(populated_world):
    ...
    assert isinstance(correct_processor_a, ECS.Processor)
    populated_world.add_processor(correct_processor_a)
    assert len(populated_world._processors) == 1
    assert isinstance(populated_world._processors[0], ECS.Processor)
...
def test_event_dispatch_no_handlers():
    ECS.dispatch_event("foo")
    ECS.dispatch_event("foo", 1)
    ECS.dispatch_event("foo", 1, 2)
    ECS.event_registry.clear()


def test_event_dispatch_one_arg():
    ECS.set_handler("foo", myhandler_onearg)
    ECS.dispatch_event("foo", 1)
    ECS.event_registry.clear()


def test_event_dispatch_two_args():
    ECS.set_handler("foo", myhandler_twoargs)
    ECS.dispatch_event("foo", 1, 2)
    ECS.event_registry.clear()


def test_event_dispatch_incorrect_args():
    ECS.set_handler("foo", myhandler_noargs)
    assert pytest.raises(TypeError), ECS.dispatch_event("foo")
    ECS.event_registry.clear()


def test_set_methoad_as_handler_in_init():

    class MyClass(ECS.Processor):

        def __init__(self):
            ECS.set_handler("foo", self._my_handler)
...
def test_set_instance_methoad_as_handler():
    ...
    myclass = MyClass()
    ECS.set_handler("foo", myclass.my_handler)
    ECS.dispatch_event("foo")
    ECS.event_registry.clear()
...
class CorrectProcessorA(ECS.Processor):

    def process(self):
        pass


class CorrectProcessorB(ECS.Processor):
...
class CorrectProcessorC(ECS.Processor):

    def process(self):
        pass


class ArgsProcessor(ECS.Processor):

    def process(self, spam):
        pass


class KwargsProcessor(ECS.Processor):
